...
export const Header = ({ searchQuery, setSearchQuery, isDarkMode, setIsDarkMode }: HeaderProps) => {
  ...
  return (
    <>
      {/* Fixed header – always stays on screen while scrolling */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-xl border-b border-border/50 supports-[backdrop-filter]:bg-background/80">
      ...
      {/* Spacer so page content is not hidden under the fixed header */}
      {/* Desktop: h-16 only | Mobile: h-16 + search row (~3rem) */}
      <div
        className="w-full shrink-0 md:h-16 h-[7.25rem]"
        aria-hidden="true"
      />
    </>
  );
